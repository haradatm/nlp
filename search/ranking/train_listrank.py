...
class ListRank(Chain):
    def __init__(self, n_dim, n_units):
        super(ListRank, self).__init__(
    ...
    def __call__(self, x, t, train=True):
        ...
        loss = permutation_probability_loss(y, t)

        acc_map = mean_average_precision(y.data, t.data, k=20)
        acc_ndcg = n_discount_cumulative_count(y.data, t.data, k=20)

        return loss, acc_map, acc_ndcg
...
if __name__ == '__main__':
    ...
    parser = ArgumentParser(description='Chainer example: ListRank')
    ...
    for epoch in xrange(1, n_epoch + 1):
        ...
        for x_batch, t_batch in sorted_gen:
            ...
            # 順伝播させて誤差と精度を算出
            loss, acc1, acc2 = model(x, t, train=True)
        ...
        for x_batch, t_batch in sorted_gen:
            ...
            # 順伝播させて誤差と精度を算出
            loss, acc1, acc2 = model(x, t, train=False)
        ...
        print('[{:>3d}] T/loss={:.6f} T/map={:.6f} T/ndcg={:.6f} T/sec={:.6f} D/loss={:.6f} D/map={:.6f} D/ndcg={:.6f} D/sec={:.6f} lr={:.6f}'.format(
            epoch, mean_train_loss, mean_train_accuracy1, mean_train_accuracy2, train_throughput, mean_test_loss, mean_test_accuracy1, mean_test_accuracy2, test_throughput, optimizer.alpha))
        sys.stdout.flush()
    ...
    if args.test:
        ...
        now = time.time()
        throuput = now - cur_at
        print('test/loss={:.6f} test/map={:.6f} test/ndcg={:.6f} test/sec={:.6f}'.format(sum_test_loss / K, sum_test_accuracy1 / K, sum_test_accuracy2 / K, throuput))
        sys.stdout.flush()
        cur_at = now
        ...
        now = time.time()
        throuput = now - cur_at
        print('test/loss={:.6f} test/acc={:.6f} test/sec={:.6f}'.format(sum_test_loss / K, sum_test_accuracy1 / K, sum_test_accuracy2 / K, throuput))
        sys.stdout.flush()
        cur_at = now
