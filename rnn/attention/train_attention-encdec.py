...
def load_comparable_data(path, vocab=None, inv_vocab=None):
    ...
    if not vocab:
        ...
        if EOL_TOKEN not in vocab:
            EOL_ID = len(vocab)
            vocab[EOL_TOKEN] = EOL_ID
            inv_vocab[id] = EOL_TOKEN
        else:
            EOL_ID = vocab[EOL_TOKEN]
...
class MyATT(chainer.Chain):
    def __init__(self, vocab_size, embed_size, hidden_size):
        super(MyATT, self).__init__(
            ...
            w5=L.Linear(hidden_size, hidden_size),
            w6=L.Linear(hidden_size, hidden_size),
            w7=L.Linear(embed_size, hidden_size),
            h3=L.StatelessLSTM(hidden_size, hidden_size),
            w8=L.Linear(hidden_size, embed_size),
    ...
    def __call__(self, x_batch, t_batch, train=True):
        ...
        c = F.tanh(self.w3(fw_list[-1][0]) + self.w4(bw_list[0][0]))
        h = F.tanh(self.w5(fw_list[-1][1]) + self.w6(bw_list[0][1]))
        ...
        for i in xrange(t_len):
            ...
            c, h = self.h3(c, h, self.w7(e))
            y = self.wf(F.tanh(self.w8(h)))
...
if __name__ == "__main__":
    ...
    for epoch in xrange(n_epoch):
        ...
        if True:
            ...
            plt.savefig('train_attention-encdec.png'.format(epoch + 1))
            # plt.show()
