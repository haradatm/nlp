...
class EMLP(nn.Module):
    ...
    def __init__(self, n_e, n_r, k, h_a, lam, gpu=False):
        ...
        self.emb_R = nn.Embedding(self.n_r, 2 * self.k * self.H_a)
        self.mlp = nn.Sequential(
            nn.ReLU(),
            nn.Linear(self.H_a, 1),
        )
    ...
    def forward(self, X):
        ...
        W_k = self.emb_R(ls).view(-1, self.H_a, 2 * self.k)

        # Forward
        phi = torch.cat([e_hs, e_ts], 1)  # M x 2k
        x = torch.bmm(W_k, phi).view(-1, self.H_a)  # M x H_a
        y = self.mlp(x)
        return y.view(-1, 1)
