...
class MyLSTM(Chain):
    def __init__(self, n_dim, n_label):
        super(MyLSTM, self).__init__(
            ...
            fc0=L.Linear(n_dim * 2, n_dim * 2),
            fc1=L.Linear(n_dim * 2, n_dim * 2),
            fc2=L.Linear(n_dim * 2, n_label),
        )
        self.n_dim = n_dim
        self.batchsize = 50
    ...
    def forward(self, x, train=True):
        uw = F.tanh(self.fc0(Variable(xp.ones((self.batchsize, self.n_dim * 2), dtype=np.float32), volatile=not train)[0:x.shape[0],]))

        h1_list = []
        c1 = Variable(xp.zeros((x.shape[0], self.n_dim), dtype=np.float32), volatile=not train)
        h1 = Variable(xp.zeros((x.shape[0], self.n_dim), dtype=np.float32), volatile=not train)
        for i in six.moves.range(x.shape[1]):
            c1, h1 = self.fwd(c1, h1, x[:, i, :])
            h1_list.append(h1)

        h2_list = []
        c2 = Variable(xp.zeros((x.shape[0], self.n_dim), dtype=np.float32), volatile=not train)
        h2 = Variable(xp.zeros((x.shape[0], self.n_dim), dtype=np.float32), volatile=not train)
        ...
        sim_list = []
        for i in six.moves.range(x.shape[1]):
            sim = F.batch_matmul(uw, ui_list[i], transa=True)
            sim_list.append(sim)

        alpha_list = []
        for i in six.moves.range(x.shape[1]):
            alpha = F.softmax(sim_list[i])
            alpha_list.append(alpha)

        si = Variable(xp.zeros((x.shape[0], self.n_dim * 2), dtype=np.float32), volatile=not train)
        for i in six.moves.range(x.shape[1]):
            si += F.reshape(F.batch_matmul(hi_list[i], alpha_list[i]), (x.shape[0], self.n_dim * 2))
...
if __name__ == '__main__':
    ...
    parser = ArgumentParser(description='Chainer example: MyLSTM')
