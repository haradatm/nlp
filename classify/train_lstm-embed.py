...
"""Chainer example: LSTM Neural Networks for Sentence Classification
...
class MyLSTM(Chain):
    ...
    def forward(self, x, train=True):
        ...
        # LSTM を行った結果を結合する
        concat = F.concat((h1, h2), axis=1)

        # 結合した結果に Dropout をかける
        h3 = F.dropout(F.tanh(self.fc1(concat)), ratio=0.5, train=train)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output',    '-o', default='model-lstm-embed-sort',  type=str, help='output directory')
    ...
    # Prepare LSTM model
    model = MyLSTM(n_vocab, n_dim, n_label)
