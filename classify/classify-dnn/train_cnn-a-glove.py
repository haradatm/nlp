...
"""Chainer example: Convolutional Neural Networks for Sentence Classification plus Self-Attention Mechanism with GloVe pre-trained embedding.
...
class CNNClassifier(chainer.Chain):
    ...
    def __init__(self, n_layers, n_vocab, n_units, n_class, dropout=0.1, pre_embed=None):
        ...
        with self.init_scope():
            ...
            self.cnn_w3 = L.Convolution2D(n_units, n_units // 3, ksize=(3, 1), stride=1, pad=(2, 0), nobias=True)
            self.cnn_w4 = L.Convolution2D(n_units, n_units // 3, ksize=(4, 1), stride=1, pad=(3, 0), nobias=True)
            self.cnn_w5 = L.Convolution2D(n_units, n_units // 3, ksize=(5, 1), stride=1, pad=(4, 0), nobias=True)
    ...
    def predict(self, xs, softmax=False, argmax=False):
        ...
        h_a = F.split_axis(h_a_reshape, len(xs), 0)     # (40, 1), (40, 1), (40, 1) ... n=50
        h_list = []
...
if __name__ == '__main__':
    ...
    parser = ArgumentParser(description='Chainer example: CNN+Attention Classifier w/GloVe')
    ...
    parser.add_argument('--out',       '-o', default='model-cnn_a-embed',  type=str, help='output directory')
    ...
    # Setup model
    model = CNNClassifier(n_layers=1, n_vocab=n_vocab, n_units=n_units, dropout=0.4, n_class=n_class, pre_embed=pre_embed)
    if args.gpu >= 0:
        model.to_gpu()
