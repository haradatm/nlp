...
class BoWClassifier(chainer.Chain):
    ...
    def __init__(self, n_layers, n_vocab, n_units, n_class, dropout=0.1, pre_embed=None):
        ...
        with self.init_scope():
            ...
            self.output = L.Linear(None, n_class)
    ...
    def predict(self, xs1, xs2, xs3, softmax=False, argmax=False):
        ...
        x_len = self.xp.array([len(x) for x in xs1], np.int32)[:, None]
        ex_block = F.sum(embedding, axis=1) / x_len

        encodings = F.dropout(ex_block, ratio=self.dropout)
...
if __name__ == '__main__':
    ...
    parser = ArgumentParser(description='Chainer example: BoW Classifier w/BERT')
    ...
    parser.add_argument('--out',       '-o', default='model-bow-bert',  type=str, help='output directory')
    ...
    # Setup model
    model = BoWClassifier(n_layers=1, n_vocab=n_vocab, n_units=n_units, dropout=0.4, n_class=n_class, pre_embed=bert)
    if args.gpu >= 0:
        model.to_gpu()
