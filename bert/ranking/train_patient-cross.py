...
class BertRanking(chainer.Chain):
    ...
    def __call__(self, q_x1, q_x2, q_x3, d_x1, d_x2, d_x3, n_x1, n_x2, n_x3, labels):
        y_0 = self.forward(q_x1, q_x2, q_x3)
        y_1 = self.forward(d_x1, d_x2, d_x3)
        y_2 = self.forward(n_x1, n_x2, n_x3)
        logits_d1 = F.sum((y_0 * y_1), axis=1, keepdims=True)
        logits_d2 = F.sum((y_0 * y_2), axis=1, keepdims=True)
        logits = F.concat((logits_d2, logits_d1), axis=1)
        loss = F.hinge(logits, labels)
...
def main():
    ...
    parser = ArgumentParser(description='BERT example: Text Retrieval (by cross-entropy loss)')
    ...
    parser.add_argument('--out', '-o', default='results_patient-8', type=str, help='output directory')
    ...
    for epoch in range(args.start_epoch, args.epoch + 1):
        ...
        for q_x1, q_x2, q_x3, p_x1, p_x2, p_x3, n_x1, n_x2, n_x3 in train_iter:
            ...
            r = to_device(args.gpu, np.array([1] * N).astype('i'))

            # 勾配を初期化
            model.cleargrads()

            # 順伝播させて誤差と精度を算出
            loss, acc = model(q_x1, q_x2, q_x3, p_x1, p_x2, p_x3, n_x1, n_x2, n_x3, r)
