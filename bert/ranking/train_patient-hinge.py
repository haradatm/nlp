...
class BertClassifier(chainer.Chain):
    def __init__(self, bert):
        ...
        with self.init_scope():
            self.bert = bert
            self.output = Linear3D(None, 2, initialW=chainer.initializers.Normal(0.02))
    ...
    def __call__(self, input_ids, input_mask, token_type_ids, labels):
        ...
        accuracy = F.accuracy(logits, labels)
...
def eval_batch_iter(queries, docs, batch_size):

    data = []
    for q_id in range(len(queries)):
        for d_id in range(len(docs)):
            q_x1, q_x2, q_x3 = queries[q_id]
            d_x1, d_x2, d_x3 = docs[d_id]
            data.append((q_id, q_x1, q_x2, q_x3, d_x1, d_x2, d_x3))
...
def main():
    ...
    parser = ArgumentParser(description='BERT example: Text Retrieval (by hinge loss)')
    ...
    parser.add_argument('--out', '-o', default='results_patient-7', type=str, help='output directory')
    ...
    train_docs, train_doc_ids = load_data(args.train_docs, tokenizer, bert_config, type_id=1)
    ...
    test_docs, test_doc_ids = load_data(args.test_docs, tokenizer, bert_config, type_id=1)
    ...
    for epoch in range(args.start_epoch, args.epoch + 1):
        ...
        for q_x1, q_x2, q_x3, d_x1, d_x2, d_x3, r in train_iter:
            N = len(q_x1)
            q_x1 = F.pad_sequence(q_x1, length=None, padding=0)
            q_x2 = F.pad_sequence(q_x2, length=None, padding=0)
            q_x3 = F.pad_sequence(q_x3, length=None, padding=0)
            d_x1 = F.pad_sequence(d_x1, length=None, padding=0)
            d_x2 = F.pad_sequence(d_x2, length=None, padding=0)
            d_x3 = F.pad_sequence(d_x3, length=None, padding=0)
            x1 = to_device(args.gpu, F.concat((q_x1, d_x1), axis=1).array).astype('i')
            x2 = to_device(args.gpu, F.concat((q_x2, d_x2), axis=1).array).astype('f')
            x3 = to_device(args.gpu, F.concat((q_x3, d_x3), axis=1).array).astype('i')
            r = to_device(args.gpu, np.array(r, 'i'))

            # 勾配を初期化
            model.cleargrads()

            # 順伝播させて誤差と精度を算出
            loss, acc = model(x1, x2, x3, r)
            sum_train_loss += float(loss.data) * N
            sum_train_accuracy1 += float(acc.data) * N
            sum_train_accuracy2 += .0
            K += N
            ...
            with chainer.no_backprop_mode(), chainer.using_config('train', False):
                with open(os.path.join(args.out, 'train.results.txt'), 'w') as f:
                    train_iter = eval_batch_iter(train_queries, train_docs, args.batchsize)
                    results = None

                    for q_id, q_x1, q_x2, q_x3, d_x1, d_x2, d_x3 in train_iter:
                        q_x1 = F.pad_sequence(q_x1, length=None, padding=0)
                        q_x2 = F.pad_sequence(q_x2, length=None, padding=0)
                        q_x3 = F.pad_sequence(q_x3, length=None, padding=0)
                        d_x1 = F.pad_sequence(d_x1, length=None, padding=0)
                        ...
                        x2 = to_device(args.gpu, F.concat((q_x2, d_x2), axis=1).array).astype('f')
                        x3 = to_device(args.gpu, F.concat((q_x3, d_x3), axis=1).array).astype('i')
                        y = F.softmax(model.forward(x1, x2, x3))
                        result = np.hstack((np.array(q_id)[:, None], cuda.to_cpu(y.data)[:, 1][:, None]))
                        results = result if results is None else np.vstack((results, result))

                    for i in range(len(train_que_ids)):
                        score = results[results[:, 0] == float(i)][:, 1]
                        for j, k in enumerate(score.argsort()[-1:-(args.max_eval + 1):-1]):
                            f.write("{}\tQ0\t{}\t{}\t{:.6f}\tSTANDARD\n".format(train_que_ids[i], train_doc_ids[k], j, score[k]))
                            f.flush()
        ...
        # with chainer.no_backprop_mode(), chainer.using_config('train', False):
        #     for q_x1, q_x2, q_x3, d_x1, d_x2, d_x3, r in test_iter:
        #         N = len(q_x1)
        #         q_x1 = F.pad_sequence(q_x1, length=None, padding=0)
        #         q_x2 = F.pad_sequence(q_x2, length=None, padding=0)
        #         q_x3 = F.pad_sequence(q_x3, length=None, padding=0)
        ...
        #         sum_test_accuracy1 += float(acc.data) * N
        #         sum_test_accuracy2 += .0
        #         K += N
        #
        #     with open(os.path.join(args.out, 'test.results.txt'), 'w') as f:
        #         test_iter = eval_batch_iter(test_queries, test_docs, args.batchsize)
        #         results = None
        #
        #         for q_id, q_x1, q_x2, q_x3, d_x1, d_x2, d_x3 in test_iter:
        #             q_x1 = F.pad_sequence(q_x1, length=None, padding=0)
        #             q_x2 = F.pad_sequence(q_x2, length=None, padding=0)
        #             q_x3 = F.pad_sequence(q_x3, length=None, padding=0)
        #             d_x1 = F.pad_sequence(d_x1, length=None, padding=0)
        ...
        #             results = result if results is None else np.vstack(results, result)
        #
        #         for i in range(len(test_que_ids)):
        #             score = results[results[:, 0] == float(i)][:, 1]
        #             for j, k in enumerate(score.argsort()[-1:-(args.max_eval + 1):-1]):
        #                 f.write("{}\tQ0\t{}\t{}\t{:.6f}\tSTANDARD\n".format(test_que_ids[i], test_doc_ids[k], j, score[k]))
        #                 f.flush()
        #
