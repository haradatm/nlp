...
class ERMLP(chainer.Chain):
    ...
    def __init__(self, n_e, n_r, k, lam, gpu=False):
        ...
        with self.init_scope():
            ...
            self.l1 = L.Linear(self.k * 3, self.k)
            self.l2 = L.Linear(self.k, 1)
    ...
    def forward(self, X):
        ...
        # Forward
        phi = F.concat([e_hs, e_ts, e_ls], 1)  # M x 3k
        y = self.l2(F.tanh(self.l1(phi)))
...
def main():
    ...
    import argparse
    parser = argparse.ArgumentParser(description='KGE example: ERMLP')
