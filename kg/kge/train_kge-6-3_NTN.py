...
class NTN(chainer.Chain):
    ...
    def __init__(self, n_e, n_r, k, H_b, lam, gpu=False):
        ...
        with self.init_scope():
            ...
            self.emb_R = L.EmbedID(self.n_r, self.k * self.k * self.H_b, initialW=chainer.initializers.HeNormal())
            self.V = L.EmbedID(self.n_r, 2 * self.k * self.H_b, initialW=chainer.initializers.HeNormal())
            self.U = L.EmbedID(self.n_r, self.H_b, initialW=chainer.initializers.HeNormal())
            self.b = L.EmbedID(self.n_r, self.H_b, initialW=chainer.initializers.HeNormal())
    ...
    def forward(self, X):
        ...
        # M x s x k x k
        Wr = F.reshape(self.emb_R(ls), (-1, self.H_b, self.k, self.k))

        # M x s x 2k
        Vr = F.reshape(self.V(ls), (-1, self.H_b, 2*self.k))

        # M x 1 x s
        Ur = F.reshape(self.U(ls), (-1, 1, self.H_b))

        # M x s
        br = F.reshape(self.b(ls), (-1, self.H_b))
        ...
        e_hs_ = F.expand_dims(F.broadcast_to(F.expand_dims(e_hs, 1), (M, self.H_b, self.k)), 3)

        # M x s x k x 1
        # e_ts_ = e_ts.unsqueeze(1).expand(M, self.H_b, self.k, 1)
        e_ts_ = F.expand_dims(F.broadcast_to(F.expand_dims(e_ts, 1), (M, self.H_b, self.k)), 3)

        # M x s x 1
        # quad = torch.matmul(torch.matmul(e_hs_, Wr), e_ts_)
        # quad = quad.view(-1, self.H_b)  # M x s
        quad = F.reshape(F.matmul(F.matmul(e_hs_, Wr, transa=True, transb=False), e_ts_, transa=False, transb=False), (-1, self.H_b))   # M x s

        # Vr: M x s x 2k
        # [e1 e2]: M x 2k x 1
        # es = torch.cat([e_hs, e_ts], dim=1)  # M x 2k x 1
        # affine = torch.baddbmm(br, Vr, es).view(-1, self.H_b)  # M x s
        es = F.expand_dims(F.concat([e_hs, e_ts], 1), 2)
        affine = F.reshape(F.matmul(Vr, es, transa=False, transb=False), (-1, self.H_b)) + br    # M x s

        # Scores
        g = F.reshape(F.matmul(Ur, F.expand_dims(F.leaky_relu(quad + affine), 2), transa=False, transb=False), (-1, 1))
...
def main():
    ...
    import argparse
    parser = argparse.ArgumentParser(description='KGE example: NTN')
    ...
    model = NTN(n_e=n_e, n_r=n_r, k=k, H_b=n_r, lam=embeddings_lambda, gpu=False)
