...
# Network definition
class EMLP(chainer.Chain):
    """
    ER-MLP: Entity MLP
    ---------------------------
    """

    def __init__(self, n_e, n_r, k, h_a, lam, gpu=False):
        """
        E-MLP: Entity-MLP
        ---------------------------
        ...
        with self.init_scope():
            ...
            self.emb_R = L.EmbedID(self.n_r, 2 * self.k * self.H_a, initialW=chainer.initializers.HeNormal())
            # self.l1 = L.Linear(self.k * 2, self.k * 2)
            self.l2 = L.Linear(self.H_a, 1)
    ...
    def forward(self, X):
        ...
        e_hs = self.emb_E(hs)
        e_ts = self.emb_E(ts)
        W_k = F.reshape(self.emb_R(ls), (-1, 2 * self.k, self.H_a))     # M x 2k x H_a

        # Forward
        phi = F.concat([e_hs, e_ts], 1)     # M x 2k
        x = F.reshape(F.matmul(W_k, F.expand_dims(phi, 2), transa=True, transb=False), (-1, self.H_a))  # M x H_a
        y = self.l2(F.relu(x))
...
def main():
    ...
    import argparse
    parser = argparse.ArgumentParser(description='KGE example: E-MLP')
    ...
    model = EMLP(n_e=n_e, n_r=n_r, k=k, h_a=h_a, lam=embeddings_lambda, gpu=False)
