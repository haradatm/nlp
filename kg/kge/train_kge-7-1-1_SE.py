...
# Network definition
class StructuredE(chainer.Chain):
    """
    ...
    def __init__(self, n_e, n_r, k, margin, distance='l2', gpu=False):
        ...
        with self.init_scope():
            ...
            self.W_s = L.EmbedID(self.n_r, self.k, initialW=chainer.initializers.HeNormal())
            self.W_o = L.EmbedID(self.n_r, self.k, initialW=chainer.initializers.HeNormal())
    ...
    def forward(self, X):
        ...
        if self.distance == 'l1':
            out = F.sum(F.absolute(w_s * e_hs - w_o * e_ts), 1)
        else:
            out = F.sqrt(F.sum((w_s * e_hs - w_o * e_ts) ** 2, 1))
        return out
            ...
            if it % print_every == 0:
                ...
                with chainer.no_backprop_mode(), chainer.using_config('train', False):
                    ...
                    print('Iter-{}; loss: {:.4f}; train_auc: {:.4f}; val_auc: {:.4f}; time per batch: {:.2f}s'
                          .format(it, loss.data, train_acc, val_acc, end - start))
                    sys.stdout.flush()

                    import plot_util
                    plot_util.plot(loss.data, train_acc, .0, .0, .0, val_acc, '{}.png'.format(os.path.splitext(os.path.basename(__file__))[0]))
